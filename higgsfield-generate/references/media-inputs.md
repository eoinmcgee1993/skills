...
| `brain_activity` | `video` | Virality Predictor analyzes one uploaded clip and returns a text engagement report; no prompt required. Treat "analyze this video" / "score this ad" as this video-analysis flow even though the output is text. When present, `brain_example_url` is the 3D brain asset (`.glb`) and `vertexMapBinaryUrl` is the activity map binary (`.bin`). |
