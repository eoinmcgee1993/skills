...
- One-shot create+wait: `higgsfield generate create <model> ... --wait` blocks until terminal and prints the URL itself. Prefer this over the legacy two-step `create` → `wait`. Tunables: `--wait-timeout`, `--wait-interval`. `product-photoshoot create` waits internally — no `--wait` needed.
